...
PRJ_NAME=ddd-laravel                                                # Project Name
...
if [ $# -eq 0 ]; then
    ...
    # ライブラリインストール
    docker run --rm --env WWWUSER=1000 -v `pwd`:/var/www/html ${PRJ_NAME}/${APP_CONTAINER} sh -c "composer install"
    docker run --rm --env WWWUSER=1000 -v `pwd`:/var/www/html ${PRJ_NAME}/${APP_CONTAINER} sh -c "npm ci"
